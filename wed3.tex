...
\lstdefinelanguage{JavaScript}{
	keywords={break, case, catch, continue, debugger, default, delete, do, else, false, finally, for, function, if, in, instanceof, new, null, return, switch, this, throw, true, try, typeof, var, let, void, while, with, private},
	morecomment=[l]{//},
	morecomment=[s]{/*}{*/},
	morestring=[b]',
	morestring=[b]",
...
\lstset{ %
	...
	aboveskip=0pt,				% sets the default margin above (default: \medskipamount)
	belowskip=0pt,				% sets the default margin below (default: \medskipamount)
...
% Waste less space with itemizations (Auflistungen)
...
% Waste less space with enumerations
...
% Waste less space with multicols (specially embedded)
\newenvironment{tight-multicols}[1]{%
	\setlength{\columnsep}{1px}%
	\setlength{\columnseprule}{0px}%
	\setlength{\multicolsep}{0px}
	\begin{multicols}{#1}
	}
{ \end{multicols}}
...
% Waste less space with subparagraphs (i.e. Less top and bottom margin )
\titlespacing*{\subparagraph} {0pt}{0.7ex plus 1ex minus .2ex}{0.5ex plus .2ex}
...
\end{tight-itemize}
\subparagraph{Client-side routing concepts}
\begin{tight-itemize}
...
\end{tight-itemize}
\subtopic{Modules}
Modules export features (directives, services, \ldots) required by other modules. NOT to be confused with ES6 Modules (ES6=per-file; Angular=logic block composed of multiple ES6 Modules). Angular ships as multiple library modules (all with the @angular-prefix). As an ES6 module, the module library provides single export wih all containing feature s(also known as \textbf{barrel} export).
\begin{tight-itemize}
...
\maintopic{Design \& Canvas}
Es gibt vor allem SVG, Canvas und WebGL. Wir setzen Fokus auf SVG und Canvas. Canvas wird bei Bing Maps verwendet. Eine Vektor Grafik wird erst vergrössert und dann rasterisiert (umgekehrt eher schwierig). \texttt{Polyline} ist ein Zug von X-Y Punkte, welche mit einer Linie verbunden wird. \texttt{Polygon} stellt sicher dass der erste Punkt und der letzte Punkt abgeschlossen wird (d.h. verbunden). \texttt{Path} ist das mächtigste Werkzeug von Vektor Grafiken. SVGs können als Bild oder als Objekt in HTML eingebunden werden. Als Bild verlieren sie die ganzen interaktionsmöglichkeiten. SVG hat Defaultgrösse von 300px auf 150px (nicht Standardisiert).
\begin{lstlisting}
<svg viewBox="0 0 200 200" <!-- x y width height -->
	preserveAspectRatio="none"> <!-- xMaxYMin, xMidYMid slice/meet -->
	<style>
		.alert{ fill:red; }
	</style>
...
\begin{lstlisting}
<canvas id="painting">Fallback</canvas>
<script>
...
window.requestAnimationFrame(paint); // Starts everything
\end{lstlisting}
